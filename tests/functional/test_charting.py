...
def test_plot_stock_price_chart(mock_mpf_plot, mock_yf) -> None:
...
@patch("os.path.isdir")
def test_get_share_performance(mock_isdir, mock_plt, mock_yf) -> None:
...
@patch("finrobot.functional.charting.plt")
@patch("os.path.isdir")
def test_get_pe_eps_performance(mock_isdir, mock_plt, mock_yf) -> None:
    mock_plt.subplots.return_value = (MagicMock(), MagicMock())
