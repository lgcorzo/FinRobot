...
def test_get_filing(mock_environ, mock_session_cls):
...
@patch("finrobot.data_access.data_source.filings_src.prepline_sec_filings.fetch.requests.Session")
def test_get_forms_by_cik(mock_session_cls):
    mock_session_instance = mock_session_cls.return_value
    mock_response = MagicMock()
    mock_response.status_code = 200
...
def test_get_recent_acc_by_cik(mock_get_acc, mock_get_sess, mock_session):
...
@patch("finrobot.data_access.data_source.filings_src.prepline_sec_filings.fetch._get_recent_acc_num_by_cik")
def test_get_recent_cik_and_acc_by_ticker(mock_get_acc, mock_get_cik, mock_get_sess, mock_session):
    mock_get_sess.return_value = mock_session
...
def test_get_form_by_ticker(mock_get_form_cik, mock_get_cik, mock_get_sess, mock_session):
    mock_get_sess.return_value = mock_session
    mock_get_cik.return_value = "12345"
