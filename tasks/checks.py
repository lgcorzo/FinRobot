...
def poetry(ctx: Context) -> None:
    ...
    ctx.run('echo "Skipping poetry check"')


@task
def format(ctx: Context) -> None:
    """Check the formats with ruff."""
    ctx.run("ruff format --check src/ tasks/ tests/")


@task
def type(ctx: Context) -> None:
    """Check the types with mypy."""
    ctx.run("mypy src/ tasks/ tests/")


@task
def code(ctx: Context) -> None:
    """Check the codes with ruff."""
    ctx.run("ruff check src/ tasks/ tests/")


@task
def test(ctx: Context) -> None:
    """Check the tests with pytest."""
    ctx.run("pytest --numprocesses='auto' tests/")


@task
def security(ctx: Context) -> None:
    """Check the security with bandit."""
    ctx.run("bandit --recursive --configfile=pyproject.toml src/")


@task
def coverage(ctx: Context) -> None:
    """Check the coverage with coverage."""
    ctx.run("pytest --numprocesses='1' --cov=src/ --cov-fail-under=40 tests/")
